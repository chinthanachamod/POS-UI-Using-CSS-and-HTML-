...
const loadCustomerTable = () => {
    ...
    customer_array.map((item, index) => {
        console.log(item);
        let data = `<tr><td>${item._id}</td><td>${item._customer_name}</td><td>${item._mobile}</td><td>${item._email}</td><td>${item.address}</td></tr>`
        let set = `<tr>
            <td>${item._id}</td>
...
// Add/Save Customer Button
$("#customer_add_btn").on("click", function() {
    let customer_name = $('#customerName').val(); // empty
    let mobile = $('#mobile').val(); // empty, format
    let email = $('#customerEmail').val(); // empty, format
